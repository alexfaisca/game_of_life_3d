...
char ***gen_initial_grid(int64_t n, float density, int input_seed,
  ...
  printf("\t-> SEED %d\n", seed);
#endif
  if (rank != 0 && MPI_Recv(&seed, 1, MPI_UNSIGNED, rank - 1, 0, MPI_COMM_WORLD,
  ...
  if (!(counter = calloc(sizeof(int), omp_get_max_threads() * (N_SPECIES + 1))))
    exit(1);
  ...
  if (rank != world_size - 1 && world_size > 1)
    if (MPI_Send(&seed, 1, MPI_UNSIGNED, rank + 1, 0, MPI_COMM_WORLD) !=
        MPI_SUCCESS)
      exit(1);
...
void synchronize_extremities(int64_t n, int world_size, int rank) {
  ...
  if (rank == 0) {
    if (MPI_Send(&(last_row[0][0][0]), t, MPI_BYTE, 1, 0, MPI_COMM_WORLD) !=
        MPI_SUCCESS)
      exit(1);
    if (MPI_Recv(&(last_row[1][0][0]), t, MPI_BYTE, 1, 0, MPI_COMM_WORLD,
                 MPI_STATUS_IGNORE) != MPI_SUCCESS)
      exit(1);
    if (world_size % 2) {
      if (MPI_Send(&(grid[1][0][0]), t, MPI_BYTE, world_size - 1, 0,
                   MPI_COMM_WORLD) != MPI_SUCCESS)
        exit(1);
      if (MPI_Recv(&(grid[0][0][0]), t, MPI_BYTE, world_size - 1, 0,
    ...
    if (world_size % 2) {
      if (MPI_Send(&(grid[1][0][0]), t, MPI_BYTE, rank - 1, 0,
                   MPI_COMM_WORLD) != MPI_SUCCESS)
        exit(1);
      if (MPI_Recv(&(grid[0][0][0]), t, MPI_BYTE, rank - 1, 0, MPI_COMM_WORLD,
                   MPI_STATUS_IGNORE) != MPI_SUCCESS)
        exit(1);
      if (MPI_Recv(&(last_row[1][0][0]), t, MPI_BYTE, 0, 0, MPI_COMM_WORLD,
                   MPI_STATUS_IGNORE) != MPI_SUCCESS)
        exit(1);
      if (MPI_Send(&(last_row[0][0][0]), t, MPI_BYTE, 0, 0, MPI_COMM_WORLD) !=
          MPI_SUCCESS)
        exit(1);
    } else {
      if (MPI_Recv(&(grid[0][0][0]), t, MPI_BYTE, rank - 1, 0, MPI_COMM_WORLD,
    ...
    if (MPI_Recv(&(grid[0][0][0]), t, MPI_BYTE, rank - 1, 0, MPI_COMM_WORLD,
                 MPI_STATUS_IGNORE) != MPI_SUCCESS)
      exit(1);
    if (MPI_Send(&(grid[1][0][0]), t, MPI_BYTE, rank - 1, 0, MPI_COMM_WORLD) !=
        MPI_SUCCESS)
      exit(1);
    if (MPI_Recv(&(last_row[1][0][0]), t, MPI_BYTE, rank + 1, 0, MPI_COMM_WORLD,
                 MPI_STATUS_IGNORE) != MPI_SUCCESS)
      exit(1);
    if (MPI_Send(&(last_row[0][0][0]), t, MPI_BYTE, rank + 1, 0,
                 MPI_COMM_WORLD) != MPI_SUCCESS)
      exit(1);
  } else {
    if (MPI_Send(&(last_row[0][0][0]), t, MPI_BYTE, rank + 1, 0,
...
void simulation_synchronized(int64_t gen, int64_t n, int world_size, int rank) {
  char **swap_space;
  int *count;
  omp_lock_t lck_total;
  int64_t x, y, z, c, s, i, g, xx, neighbours;
  int64_t total[N_SPECIES] = {0}, aux[N_SPECIES] = {0};
  xx = world_size > 1 ? n / world_size + (rank < n % world_size) : n;
  omp_init_lock(&lck_total);
  if (world_size > 1) {
#ifdef DEBUG
    ...
    for (x = 0; x < N_SPECIES; total[x] = stats[x].c, x++)
      ;
    if (MPI_Reduce(total, aux, N_SPECIES, MPI_INT64_T, MPI_SUM, 0,
                   MPI_COMM_WORLD) != MPI_SUCCESS)
      exit(1);
#ifdef DEBUG
    printf("After stat sync: ");
#endif
    for (x = 0; x < N_SPECIES; total[x] = aux[x] = 0, x++)
      stats[x] = (Statistics){aux[x], 0}
#ifdef DEBUG
      ,
      printf("%ld ", stats[x].c);
    printf("\n");
  ...
  for (g = 0;; g++) {
    if (world_size > 1)
      synchronize_extremities(n, world_size, rank);
    else
      singleton_extremeties(n);
#ifdef DEBUG
    print_grid(g, n);
#endif
    if (g >= gen)
...
#pragma omp parallel
      {
#pragma omp for private(count, y, z, neighbours, c, s, i) collapse(2)          \
    schedule(dynamic, 1)
        for (y = 0; y < n; y++) {
          for (z = 0; z < n; z++) {
            count = counter + (N_SPECIES + 1) * omp_get_thread_num();
            neighbours = get_neighbours(n, x, y, z, count);
            for (s = 1, i = 1, c = 0; s <= N_SPECIES;
                 (counter + (N_SPECIES + 1) * omp_get_thread_num())[s++] = 0)
              if ((counter + (N_SPECIES + 1) * omp_get_thread_num())[s] > c)
                i = s, c = count[s];

            if (grid[x][y][z] == 0) {
              if (neighbours > 6 && neighbours < 11) {
                grid[xx + 3][y][z] = i;
                omp_set_lock(&lck_total);
                total[grid[xx + 3][y][z] - 1]++;
                omp_unset_lock(&lck_total);
              } else
                grid[xx + 3][y][z] = 0;
            } else if (neighbours < 5 || neighbours > 13)
              grid[xx + 3][y][z] = 0;
            else {
              grid[xx + 3][y][z] = grid[x][y][z];
              omp_set_lock(&lck_total);
              total[grid[xx + 3][y][z] - 1]++;
              omp_unset_lock(&lck_total);
            }
    ...
    if (world_size > 1) {
      if (MPI_Reduce(total, aux, N_SPECIES, MPI_INT64_T, MPI_SUM, 0,
        ...
        exit(1);
      for (x = 0; x < N_SPECIES; total[x] = aux[x] = 0, x++)
        if (aux[x] > stats[x].c && rank == 0)
          stats[x] = (Statistics){aux[x], g + 1};
    } else
...
int main(int argc, char *argv[]) {
  ...
  if (init_mpi(&argc, &argv, MPI_THREAD_MULTIPLE, &thread) != MPI_SUCCESS)
    return 1;
  ...
  MPI_Barrier(MPI_COMM_WORLD), MPI_Finalize(), MPI_Finalized(&world_size);
  return !world_size;
}
