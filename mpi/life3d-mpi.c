...
#endif
  if (rank != 0)
    MPI_Recv(&seed, 1, MPI_UNSIGNED, rank - 1, 0, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
...
void synchronize_extremities(int64_t n, int world_size, int rank) {
  ...
  if (rank == 0) {
    ...
    MPI_Recv(&(last_row[1][0][0]), t, MPI_BYTE, 1, 0, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
    if (world_size % 2) {
      MPI_Send(&(grid[1][0][0]), t, MPI_BYTE, world_size - 1, 0,
               MPI_COMM_WORLD);
      MPI_Recv(&(grid[0][0][0]), t, MPI_BYTE, world_size - 1, 0, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE);
    } else {
      MPI_Recv(&(grid[0][0][0]), t, MPI_BYTE, world_size - 1, 0, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE);
    ...
    if (world_size % 2) {
      ...
      MPI_Recv(&(grid[0][0][0]), t, MPI_BYTE, rank - 1, 0, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE);
      MPI_Recv(&(last_row[1][0][0]), t, MPI_BYTE, 0, 0, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE);
    ...
    MPI_Recv(&(grid[0][0][0]), t, MPI_BYTE, rank - 1, 0, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
    MPI_Send(&(grid[1][0][0]), t, MPI_BYTE, rank - 1, 0, MPI_COMM_WORLD);
    MPI_Recv(&(last_row[1][0][0]), t, MPI_BYTE, rank + 1, 0, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
...
void simulation_synchronized(int64_t gen, int64_t n, int world_size, int rank) {
  ...
  if (world_size > 1) {
    ...
    for (x = 0; x < N_SPECIES; total[x] = stats[x].c, x++)
      ;
    MPI_Reduce(total, count, N_SPECIES, MPI_INT64_T, MPI_SUM, 0,
               MPI_COMM_WORLD);
#ifdef DEBUG
      ...
      for (y = 0; y < n; y++) {
        for (z = 0; z < n; z++) {
          ...
          if (grid[x][y][z] == 0) {
            ...
            total[(grid[xx + 3][y][z] = grid[x][y][z]) - 1]++;
          }
        }
      }
      if (x > 1) {
